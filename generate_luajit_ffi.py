...
"""Use checkdocs prototypes to generate a LuaJIT FFI wrapper for Allegro5.

If the prototype is not yet generated do something like:
    `allegro5/python/checkdocs.py -s allegro5 -p allegro5/contrib/luajit_ffi/proto -b build_allegro_monolith/`

And create the wrapper with then something like:
    `allegro5/contrib/luajit_ffi/generate_luajit_ffi.py -o allegro5/contrib/luajit_ffi/allegro5.lua -p allegro5/contrib/luajit_ffi/proto`
"""
...
def main(argv):
    """Generate the ffi lua wrapper for allegro 5."""
    p = optparse.OptionParser()
    p.add_option("-o", "--output", help="location of generated file")
    p.add_option("-p", "--protos", help=(
        "A file with all prototypes to generate LuaJIT FFI wrappers for, one "
        "per line. \n Generate it with docs/scripts/checkdocs.py -p"))
    p.add_option("-t", "--type", help=(
        "the library type to use, e.g. debug"))
    p.add_option("-v", "--version", help=(
        "the library version to use, e.g. 5.1"))
